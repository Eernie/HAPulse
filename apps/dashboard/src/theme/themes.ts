...
export function applyTheme(name: ThemeName, mode: ThemeMode = 'light', accentHue?: number): void {
  ...
  if (accentHue !== undefined) {
    ...
    const s = 78;
    root.style.setProperty('--accent', `hsl(${accentHue}, ${s}%, ${l}%)`);
    root.style.setProperty('--accent-soft', `hsla(${accentHue}, ${s}%, ${l}%, ${isDark ? 0.16 : 0.12})`);
    root.style.setProperty('--on-accent', isDark ? '#140e04' : '#ffffff');
  }
}
